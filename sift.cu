...
struct Sift
{
  static int maxO (int w, int h, int o_min)
  {
    int o = static_cast<int> (floor (log2 (min (w, h))) - o_min - 3);
    return max (o, 1);
  }
...
#define VL_SINGLE_EPSILON 1.19209290E-07F
  double
  normalize_histogram (double *begin, double *end)
{
  double norm = 0.0;
  for (double* iter = begin ; iter != end ; ++iter)
    norm += (*iter) * (*iter);
  norm = sqrt (norm) + VL_SINGLE_EPSILON;
  for (double* iter = begin; iter != end ; ++iter)
    *iter /= norm;
  return norm;
}
...
#define atd(dbinx,dbiny,dbint) *(dpt + (dbint)*binto + (dbiny)*binyo + (dbinx)*binxo)
  ...
  std::list<SiftFeaturePoint> extract ()
  {
    std::list<SiftFeaturePoint> res;
...
private:
  ...
  void
  copy_and_downsample (double* dst,  double const* src,
...
IplImage* computeResultImage (const std::list<SiftFeaturePoint>& sift, IplImage& img,
