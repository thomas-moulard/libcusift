...
Le but d'une implémentation sur GPU est d'accélérer le calcul des points
d'intérêt d'une image. Par exemple, une application pouvant nécessiter des
calculs rapide de points SIFT est la vision robotique: on peut imaginer un
robot aillant besoin d'appliquer un traitement pour chaque image capturée par
...
tout au long de ce rapport. Cette image est disponible dans l'implémentation SIFT
...
mêmes conditions (nombre d'octave, d'échelle, \ldots) que le binaire de
``Feat'' afin que les résultats puissent être comparable. Le résultat des deux
implémentations est relativement similaire (la différence tient, par
exemple, au fait ``Feat'' utilise des \texttt{float} et cette implémentation
des \texttt{double}).


Pour compiler le projet, il est nécessaire de le placer dans le dossier
...
Cette série d'image est décomposée en octaves et en échelles (``scale'').  On
...
de cette méthode est qu'elle consomme bien plus de mémoire puisque tout est
alloué simultanément.. D'autre part, il serait souhaitable de rendre certains
appels asynchrones car l'ensemble d'une étape n'est pas forcément nécessaire
pour calculer l'étape suivante. En particulier, les dernières octaves peuvent
être calculées très rapidement car la taille de celles-ci est très inférieure à
celle de l'octave minimum calculée. Ce type de comportement est gérable via les
fonctionnalités avancées de CUDA.
